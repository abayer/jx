...
func TestSequentialWorkflow(t *testing.T) {
	...
	a, err := util.CreateTestPipelineActivity(jxClient, ns, testOrgName, testRepoName, "master", "1", myFlowName)
	assert.NoError(t, err)
	if err != nil {
		return
	}
	...
	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, stagingRepo.Owner, stagingRepo.GitRepo.Name, 1) {
		return
	}
	...
	util.AssertHasPullRequestForEnv(t, activities, a.Name, "production")
	util.AssertHasPromoteStatus(t, activities, a.Name, "production", v1.ActivityStatusTypeRunning)
	util.AssertHasPipelineStatus(t, activities, a.Name, v1.ActivityStatusTypeRunning)

	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, prodRepo.Owner, prodRepo.GitRepo.Name, 1) {
		return
	}
	if !util.AssertSetPullRequestComplete(t, fakeGitProvider, prodRepo, 1) {
		return
	}
	...
	util.AssertHasPromoteStatus(t, activities, a.Name, "staging", v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, "production", v1.ActivityStatusTypeSucceeded)
...
func TestWorkflowManualPromote(t *testing.T) {
	...
	util.AssertHasPromoteStatus(t, activities, a.Name, "production", v1.ActivityStatusTypeRunning)
	util.AssertWorkflowStatus(t, activities, a.Name, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPipelineStatus(t, activities, a.Name, v1.ActivityStatusTypeSucceeded)
...
func TestParallelWorkflow(t *testing.T) {
	...
	if err == nil {
		...
		if err == nil {
			...
			if len(spec.Steps) > 0 {
				util.AssertPromoteStep(t, &spec.Steps[0], envNameA)
			}
			if len(spec.Steps) > 1 {
				util.AssertPromoteStep(t, &spec.Steps[1], envNameB)
			}
			if len(spec.Steps) > 2 {
				util.AssertPromoteStep(t, &spec.Steps[2], envNameC)
			}
		}
	}
	...
	activities := jxClient.JenkinsV1().PipelineActivities(ns)
	util.AssertHasPullRequestForEnv(t, activities, a.Name, envNameA)
	util.AssertHasPullRequestForEnv(t, activities, a.Name, envNameB)
	util.AssertWorkflowStatus(t, activities, a.Name, v1.ActivityStatusTypeRunning)

	// lets make sure we don't create a PR for production as we have not completed the staging PR yet
	err = o.Run()
	util.AssertHasNoPullRequestForEnv(t, activities, a.Name, envNameC)
	...
	// test no PR on production until staging completed
	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, repoA.Owner, repoA.GitRepo.Name, 1) {
		return
	}
	...
	if !util.AssertSetPullRequestComplete(t, fakeGitProvider, repoA, 1) {
		return
	}
	...
	util.AssertHasNoPullRequestForEnv(t, activities, a.Name, envNameC)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameA, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameB, v1.ActivityStatusTypeRunning)
	util.AssertHasPipelineStatus(t, activities, a.Name, v1.ActivityStatusTypeRunning)

	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, repoB.Owner, repoB.GitRepo.Name, 1) {
		return
	}
	if !util.AssertSetPullRequestComplete(t, fakeGitProvider, repoB, 1) {
		return
	}
	...
	util.AssertHasPullRequestForEnv(t, activities, a.Name, envNameC)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameA, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameB, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameC, v1.ActivityStatusTypeRunning)

	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, repoC.Owner, repoC.GitRepo.Name, 1) {
		return
	}
	if !util.AssertSetPullRequestComplete(t, fakeGitProvider, repoC, 1) {
		return
	}
	...
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameA, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameB, v1.ActivityStatusTypeSucceeded)
	util.AssertHasPromoteStatus(t, activities, a.Name, envNameC, v1.ActivityStatusTypeSucceeded)
...
func TestNewVersionWhileExistingWorkflow(t *testing.T) {
	...
	aOld := a
	a, err = util.CreateTestPipelineActivity(jxClient, ns, testOrgName, testRepoName, "master", "2", myFlowName)
	...
	util.AssertWorkflowStatus(t, activities, aOld.Name, v1.ActivityStatusTypeAborted)
	...
	// test no PR on production until staging completed
	if !util.AssertSetPullRequestMerged(t, fakeGitProvider, stagingRepo.Owner, stagingRepo.GitRepo.Name, 2) {
		return
	}
	...
	if !util.AssertSetPullRequestComplete(t, fakeGitProvider, stagingRepo, 2) {
		return
	}
